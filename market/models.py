...
class User(db.Model, UserMixin):
    ...
    def password(self, plain_text_pass):
        self.password_hash = flask_bcrypt.generate_password_hash(plain_text_pass).decode('utf-8')
...
class Item(db.Model):
    ...
    description = db.Column(db.String(length=12), nullable=False)
    owner = db.Column(db.Integer(), db.ForeignKey('user.id'))  # lowecase
